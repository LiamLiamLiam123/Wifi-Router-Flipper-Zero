...
/**
 * @brief      Start a SoftAP (Access Point) so the WiFi devboard broadcasts
 *             beacon frames and the network becomes joinable from the
 *             Flipper Zero's WiFi settings.
 * @return     true if the command was sent successfully, false otherwise.
 * @param fhttp The FlipperHTTP context
 * @param ssid     The SSID (network name) to broadcast.
 * @param password The WPA2 password (empty string for an open network).
 * @param channel  The WiFi channel to broadcast on (1..13).
 * @note       Sends `[WIFI/AP/START]{\"ssid\":\"...\",\"password\":\"...\",\"channel\":N}`
 *             over UART. The FlipperHTTP firmware must implement this command.
 */
bool flipper_http_ap_start(FlipperHTTP *fhttp, const char *ssid, const char *password, uint8_t channel)
{
    if (!fhttp)
    {
        FURI_LOG_E(HTTP_TAG, "flipper_http_ap_start: Failed to get context.");
        return false;
    }
    if (!ssid || !password)
    {
        FURI_LOG_E("FlipperHTTP", "Invalid arguments provided to flipper_http_ap_start.");
        return false;
    }
    ...
    int ret = snprintf(
        ...
        "[WIFI/AP/START]{\"ssid\":\"%s\",\"password\":\"%s\",\"channel\":%u}",
        ssid, password, (unsigned)channel);

    if (ret < 0 || ret >= (int)sizeof(buffer))
    {
        FURI_LOG_E("FlipperHTTP", "Failed to format WiFi AP start command.");
        return false;
    }
...
/**
 * @brief      Stop the SoftAP / beacon broadcast previously started with
 *             flipper_http_ap_start.
 * @return     true if the command was sent successfully, false otherwise.
 * @param fhttp The FlipperHTTP context
 * @note       Sends `[WIFI/AP/STOP]` over UART.
 */
bool flipper_http_ap_stop(FlipperHTTP *fhttp)
{
    if (!fhttp)
    {
        FURI_LOG_E(HTTP_TAG, "flipper_http_ap_stop: Failed to get context.");
        return false;
    }
    return flipper_http_send_data(fhttp, "[WIFI/AP/STOP]");
}

/**
 * @brief      Query the SoftAP status from the board.
 * @return     true if the command was sent successfully, false otherwise.
 * @param fhttp The FlipperHTTP context
 * @note       Sends `[WIFI/AP/STATUS]` over UART. The response is stored in
 *             fhttp->last_response.
 */
bool flipper_http_ap_status(FlipperHTTP *fhttp)
{
    if (!fhttp)
    {
        FURI_LOG_E(HTTP_TAG, "flipper_http_ap_status: Failed to get context.");
        return false;
    }
    return flipper_http_send_data(fhttp, "[WIFI/AP/STATUS]");
}