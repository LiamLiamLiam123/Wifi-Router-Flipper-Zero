#pragma once
#include "easy_flipper/easy_flipper.h"

class WifiRouterApp;

typedef enum
{
    SettingsViewAPSSID = 0,
    SettingsViewAPPass = 1,
    SettingsViewAPChannel = 2,
} SettingsViewChoice;

class WifiRouterSettings
{
private:
    WifiRouterApp *appContext; // reference to the app context
#ifndef FW_ORIGIN_Momentum
    UART_TextInput *text_input = nullptr; // UART text input instance
#else
    TextInput *text_input = nullptr; // Original text input instance for Momentum
#endif
    std::unique_ptr<char[]> text_input_buffer;       // buffer for text input
    uint32_t text_input_buffer_size = 128;           // size of the text input buffer
    std::unique_ptr<char[]> text_input_temp_buffer;  // temporary buffer for text input
    VariableItemList *variable_item_list = nullptr;  // variable item list for settings
    VariableItem *variable_item_ap_ssid = nullptr;   // variable item for AP SSID
    VariableItem *variable_item_ap_pass = nullptr;   // variable item for AP Password
    VariableItem *variable_item_ap_channel = nullptr; // variable item for AP Channel
    ViewDispatcher **view_dispatcher_ref;            // reference to the view dispatcher

    // View IDs
    static constexpr const uint32_t WifiRouterViewIdSubmenu = 1;
    static constexpr const uint32_t WifiRouterViewIdSettings = 3;
    static constexpr const uint32_t WifiRouterViewIdTextInput = 4;

    // Backward compatibility aliases
    static constexpr const uint32_t WifiRouterViewSubmenu = WifiRouterViewIdSubmenu;
    static constexpr const uint32_t WifiRouterViewSettings = WifiRouterViewIdSettings;
    static constexpr const uint32_t WifiRouterViewTextInput = WifiRouterViewIdTextInput;

    static uint32_t callbackToSubmenu(void *context);                        // callback to switch to the main menu
    static uint32_t callbackToSettings(void *context);                       // callback to switch to the settings view
    void freeTextInput();                                                    // free the text input resources
    bool initTextInput(uint32_t view);                                       // initialize the text input for a specific view
    static void settingsItemSelectedCallback(void *context, uint32_t index); // callback for settings item selection
    bool startTextInput(uint32_t view);                                      // start the text input for a specific view
    void textUpdated(uint32_t view);                                         // update the text input based on the view
    static void textUpdatedSsidCallback(void *context);                      // callback for AP SSID text update
    static void textUpdatedPassCallback(void *context);                      // callback for AP Password text update
    static void textUpdatedChannelCallback(void *context);                   // callback for AP Channel text update

public:
    WifiRouterSettings(ViewDispatcher **view_dispatcher, WifiRouterApp *appContext);
    ~WifiRouterSettings();

    void settingsItemSelected(uint32_t index); // handle settings item selection
};